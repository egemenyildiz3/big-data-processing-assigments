...
object Part4 {
  ...
  def reverseUsingFold[A](nrs: List[A]): List[A] = {
    nrs.foldLeft(List[A]())((acc, x) => x :: acc)
  }
  ...
  def secondLargest(xs: List[Int]): Option[Int] = xs match {
    case Nil => None
    case List(_) => None
    case _ => xs.distinct.sorted.dropRight(1).lastOption
  }
  ...
  def distinctAPI[A](xs: List[A]): List[A] = {
    xs.foldLeft(List[A]()) {
      (acc, x) => if (!acc.contains(x)) x :: acc else acc
    }.reverse
  }
  ...
  def determineGoodPlayers(scrabblePlayers: List[(String, List[(String, Int)])]): List[String] = {
    ...
    def isValid(words : List[(String, Int)]) : Boolean = words match {
      case List() => false
      case head :: Nil => head._1.length > 3
      case head :: tail => head._1.length > 3 && isValid(tail)
    }
    //print(scrabblePlayers)

    scrabblePlayers.filter(pair => isValid(pair._2)).map(pair => pair._1)
  ...
  def avgOver7(scrabblePlayers: List[(String, List[(String, Int)])]): List[(String, Double)] = {
    ...
    def average(values : List[Int]) : Double = {
      values.sum.toDouble / values.size.toDouble
    }

    val goodPlayers = determineGoodPlayers(scrabblePlayers)

    scrabblePlayers.filter(player => goodPlayers.contains(player._1)).map(pair => {
      (
        pair._1,
        average(pair._2.map(_._2))
      )
    }).filter(_._2 > 7.0)
  ...
  def goodSportStudents(studentResults: List[(String, String, Option[Int])], sportStudents: List[String]): List[String] = {
    val filteredResults = studentResults.collect {
      case (student, _, Some(grade)) if sportStudents.contains(student) => (student, grade)
    }

    val studentGroups = filteredResults.groupBy(_._1)
    val goodStudents = studentGroups.collect {
      case (student, results) if results.size == 3 && results.forall(_._2 >= 6) => student
    }
  ...
  def avgGradeGoodSportStudents(studentResults: List[(String, String, Option[Int])], sportStudents: List[String]): List[(String, Double)] = {
    val goodStudents = goodSportStudents(studentResults, sportStudents)
    goodStudents.map { name =>
      val grades = studentResults.collect {
        case (`name`, _, Some(grade)) => grade
      }
      val avgGrade = grades.sum.toDouble / grades.size
      (name, avgGrade)
    }
  }
