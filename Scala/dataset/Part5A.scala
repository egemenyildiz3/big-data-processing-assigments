...
object Part5A {
  ...
  def noAge(xs: List[Array[String]]): Int = {
    xs.map(_.apply(4)).count(_.length < 1)
  }
  ...
  def highestCheckouts(xs: List[Array[String]]): Tuple3[Int, String, String] = {
    ...
    val maxCheckouts = xs.map(_.apply(2).toInt).max

    xs.find(_.apply(2).toInt == maxCheckouts) match {
      case Some(patron) => (patron(0).toInt, patron(4), patron(8))
      case None => (0, "", "")
    }
  }
  ...
  def missing_email(xs: List[Array[String]]): Map[String, Int] = {
    ...
    val patronTypes = xs.filter(
      (patron) => patron.apply(8).toLowerCase(  ) == "email" && patron.apply(9).toLowerCase() != "true"
    ).map(
      (patron) => patron.apply(1)
    )

    patronTypes.distinct.map(
      (patronType : String) => {
        (patronType, patronTypes.count(_.toLowerCase() == patronType.toLowerCase()))
      }
    ).toMap
  }
