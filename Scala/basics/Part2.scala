...
object Part2 {
  ...
  def drunkWords(xs: List[String]): List[String] = {
    ...
    def reverseWord(word: String): String = word match {
      case "" => ""
      case _ => reverseWord(word.tail) + word.head
    }

    xs match {
      case Nil => List()
      case head :: tail =>  drunkWords(tail) ::: List(reverseWord(head))
    }
  ...
  def myForAll[A](xs: List[A], f: A => Boolean): Boolean = (xs, f) match {
    case (Nil, _) => true
    case (i :: tail, _) => f(i) && myForAll(tail, f)
  }
  ...
  def myFilter[A](xs: List[A], f: A => Boolean): List[A] = {
    def filterHelper (xs: List[A], f: A => Boolean, index: Int): List[A] = (xs, index) match {
      case (Nil, _) => Nil
      case (x :: tail, i) if !f(x) => filterHelper(tail, f, i)
      case (x :: tail, i) if f(x)  => (i) match {
        case i if i % 2 == 0 => x :: filterHelper (tail, f, i + 1)
        case i if i % 2 == 1 => filterHelper (tail, f, i + 1)
      }
      case (x :: tail, i) => {
        filterHelper (tail, f, i)
      }
    }
